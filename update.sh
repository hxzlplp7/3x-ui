...
setup_ssl_certificate() {
    ...
    echo -e "${yellow}注意：必须开放 80 端口并且可以从互联网访问${plain}"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}请确保 80 端口已开放，稍后使用 x-ui 重试${plain}"
...
setup_ip_certificate() {
    ...
    echo -e "${green}正在设置 Let's Encrypt IP 证书（短效配置文件）...${plain}"
    echo -e "${yellow}注意：IP 证书有效期约 6 天，并将自动续期。${plain}"
    echo -e "${yellow}默认监听端口为 80。如果您选择其他端口，请确外部 80 端口转发到该端口。${plain}"
    ...
    if ! [[ "${WebPort}" =~ ^[0-9]+$ ]] || ((WebPort < 1 || WebPort > 65535)); then
        echo -e "${red}提供的端口无效。使用默认值 80。${plain}"
        WebPort=80
    fi
    echo -e "${green}使用端口 ${WebPort} 进行独立验证。${plain}"
    if [[ "${WebPort}" -ne 80 ]]; then
        echo -e "${yellow}提醒：Let's Encrypt 仍然连接端口 80；请将外部 80 端口转发到 ${WebPort}。${plain}"
    fi

    # Ensure chosen port is available
    while true; do
        if is_port_in_use "${WebPort}"; then
            echo -e "${yellow}端口 ${WebPort} 目前被占用。${plain}"

            local alt_port=""
            read -rp "输入另一个端口用于 acme.sh 独立监听 (留空中止): " alt_port
            alt_port="${alt_port// /}"
            if [[ -z "${alt_port}" ]]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}请确保端口 ${WebPort} 可达 (或从外部端口 80 转发)${plain}"
    ...
    echo -e "${green}证书有效期约 6 天，并通过 acme.sh cron 任务自动续期。${plain}"
...
ssl_cert_issue() {
    ...
    if [ "${currentCert}" == "${domain}" ]; then
        ...
        echo -e "${red}系统已存在此域名的证书。无法再次签发。${plain}"
    ...
    read -rp "您想将此证书应用于面板吗？(y/n): " setPanel
...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}3.${plain} 自定义 SSL 证书（已有文件路径）"
    echo -e "${blue}注意：${plain} 选项 1 和 2 需要开放 80 端口。选项 3 需要手动输入路径。"
    read -rp "请选择一个选项（默认为 2 - IP）： " ssl_choice
    ...
    2)
        ...
        echo -e "${green}正在为 IP 使用 Let's Encrypt 证书（短效配置文件）...${plain}"
        
        # Ask for optional IPv6
        local ipv6_addr=""
        read -rp "您是否有要包含的 IPv6 地址？(留空跳过): " ipv6_addr
    ...
    3)
        ...
        echo -e "${yellow}注意：您需要自行负责外部续期这些文件。${plain}"
...
config_after_update() {
    ...
    if [[ -z "$existing_cert" ]]; then
        ...
        if [[ -z "${server_ip}" ]]; then
            echo -e "${red}无法检测服务器 IP${plain}"
            echo -e "${yellow}请使用以下命令手动配置 SSL: x-ui${plain}"
            return
        fi
...
update_x-ui() {
    ...
    tag_version=$(${curl_bin} --connect-timeout 5 --max-time 10 -Ls "https://api.github.com/repos/MHSanaei/3x-ui/releases/latest" 2>/dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    if [[ ! -n "$tag_version" ]]; then
        echo -e "${yellow}尝试使用 IPv4 获取版本...${plain}"
        tag_version=$(${curl_bin} --connect-timeout 5 --max-time 10 -4 -Ls "https://api.github.com/repos/MHSanaei/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    fi
    if [[ ! -n "$tag_version" ]]; then
        echo -e "${yellow}尝试通过代理获取版本...${plain}"
        tag_version=$(${curl_bin} -4 -Ls -o /dev/null -w '%{url_effective}' "${GH_PROXY}https://github.com/MHSanaei/3x-ui/releases/latest" | grep -oP '/tag/\K[^/]+')
        if [[ ! -n "$tag_version" ]]; then
    ...
    if [[ -e ${xui_folder}/ ]]; then
        ...
        if [[ $release == "alpine" ]]; then
            if [ -f "/etc/init.d/x-ui" ]; then
                ...
                echo -e "${green}正在移除旧的服务单元版本...${plain}"
            ...
            if [ -f "${xui_service}/x-ui.service" ]; then
                ...
                echo -e "${green}正在移除旧的 systemd 单元版本...${plain}"
    ...
    ${curl_bin} -fLRo /usr/bin/x-ui ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.sh >/dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        echo -e "${yellow}尝试使用 IPv4 获取 x-ui...${plain}"
        ${curl_bin} -4fLRo /usr/bin/x-ui ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.sh >/dev/null 2>&1
        if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        ...
        ${curl_bin} -fLRo /etc/init.d/x-ui ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.rc >/dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            ${curl_bin} -4fLRo /etc/init.d/x-ui ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.rc >/dev/null 2>&1
            if [[ $? -ne 0 ]]; then
        ...
        if [ -f "x-ui.service" ]; then
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.service.debian >/dev/null 2>&1
                    ;;
                    arch | manjaro | parch)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.service.arch >/dev/null 2>&1
                    ;;
                    *)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.service.rhel >/dev/null 2>&1
                    ;;
                esac
