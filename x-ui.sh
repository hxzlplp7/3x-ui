...
# check root
[[ $EUID -ne 0 ]] && LOGE "错误: 请使用 root 用户运行此脚本！\n" && exit 1
...
install() {
    bash <(curl -Ls https://ghfast.top/https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    confirm "此功能将更新 x-ui 所有组件到最新版本，数据不会丢失。是否继续？" "y"
    if [[ $? != 0 ]]; then
        LOGE "已取消"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://ghfast.top/https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/update.sh)
    if [[ $? == 0 ]]; then
        LOGI "更新完成，面板已自动重启"
        before_show_menu
    fi
...
update_menu() {
    ...
    curl -fLRo /usr/bin/x-ui https://ghfast.top/https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://ghfast.top/https://raw.githubusercontent.com/hxzlplp7/3x-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo ""
    echo -e "卸载成功。\n"
    echo "如需重新安装此面板，请使用以下命令:"
    echo -e "${green}bash <(curl -Ls https://ghfast.top/https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/install.sh)${plain}"
    echo ""
...
reset_webbasepath() {
    echo -e "${yellow}正在重置面板路径${plain}"

    read -rp "确定要重置面板路径吗？(y/n): " confirm
    ...
    echo -e "${green}请使用新的面板路径访问面板。${plain}"
    restart
}
    ...
    if [[ -n "$existing_cert" ]]; then
        ...
        if [[ "$gen_ssl" == "y" || "$gen_ssl" == "Y" ]]; then
            ...
            if [[ $? -eq 0 ]]; then
                ...
                LOGE "IP 证书设置失败。"
                echo -e "${yellow}您可以通过选项 18（SSL 证书管理）重试。${plain}"
                start >/dev/null 2>&1
            fi
        else
            echo -e "${yellow}访问 URL: http://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
            echo -e "${yellow}为了安全，请通过选项 18（SSL 证书管理）配置 SSL 证书${plain}"
        fi
    fi
}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            LOGE "面板启动失败，可能是因为启动时间超过2秒，请稍后查看日志信息"
        fi
    fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        if [[ $? == 1 ]]; then
            ...
            LOGE "面板停止失败，可能是因为停止时间超过2秒，请稍后查看日志信息"
        fi
    fi
...
restart() {
    ...
    if [[ $? == 0 ]]; then
        ...
        LOGE "面板重启失败，可能是因为启动时间超过2秒，请稍后查看日志信息"
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
update_shell() {
    curl -fLRo /usr/bin/x-ui -z /usr/bin/x-ui https://ghfast.top/https://github.com/hxzlplp7/3x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
open_ports() {
    ...
    if ! [[ $ports =~ ^([0-9]+|[0-9]+-[0-9]+)(,([0-9]+|[0-9]+-[0-9]+))*$ ]]; then
        echo "错误: 无效输入。请输入逗号分隔的端口或端口范围（如 80,443,2053 或 400-500）。" >&2
        exit 1
    fi
...
ssl_cert_issue_main() {
    ...
    2)
        ...
        if [ -z "$domains" ]; then
            ...
            echo "已有域名:"
            echo "$domains"
            read -rp "请从列表中输入要吁消证书的域名: " domain
...
ssl_cert_issue_for_ip() {
    ...
    LOGI "使用 Let's Encrypt 短期配置（约 6 天有效期，自动续期）"
...
ssl_cert_issue_CF() {
    ...
    LOGI "3. 域名。"
    LOGI "4. 证书签发后，系统将提示您是否将其设置为面板证书（可选）。"
...
show_banlog() {
    ...
    if [[ -f "${iplimit_banned_log_path}" ]]; then
        ...
        echo -e "${red}未在以下位置找到封禁日志文件： ${iplimit_banned_log_path}${plain}"
    fi
...
EOF
    ...
    echo -e "${green}已创建 IP 限制监狱文件，封禁时长为 ${bantime} 分钟。${plain}"
}
...
iplimit_remove_conflicts() {
    ...
    for file in "${jail_files[@]}"; do
        ...
        if test -f "${file}" && grep -qw '3x-ipl' ${file}; then
            sed -i "/\[3x-ipl\]/,/^$/d" ${file}
            echo -e "${yellow}正在移除监狱 (${file}) 中的 [3x-ipl] 冲突配置！${plain}\n"
        fi
    done
}
