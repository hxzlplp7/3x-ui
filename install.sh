...
setup_ip_certificate() {
    ...
    echo -e "${yellow}acme.sh 将在过期前自动续期并重启 x-ui。${plain}"
    return 0
}
...
config_after_install() {
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            read -rp "您想自定义面板端口设置吗？（如果不，将应用随机端口）[y/n]: " config_confirm
            ...
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${yellow}为了安全，所有面板都需要 SSL 证书。${plain}"
            ...
            local config_webBasePath=$(gen_random_string 18)
            echo -e "${yellow}Web 路径丢失或太短。正在生成新的...${plain}"
            ${xui_folder}/x-ui setting -webBasePath "${config_webBasePath}"
            echo -e "${green}新 Web 路径: ${config_webBasePath}${plain}"
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl --connect-timeout 5 --max-time 10 -Ls "https://api.github.com/repos/MHSanaei/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}尝试使用 IPv4 获取版本...${plain}"
            tag_version=$(curl --connect-timeout 5 --max-time 10 -4 -Ls "https://api.github.com/repos/MHSanaei/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        fi
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}尝试通过代理获取版本...${plain}"
            tag_version=$(curl -4 -Ls -o /dev/null -w '%{url_effective}' "${GH_PROXY}https://github.com/MHSanaei/3x-ui/releases/latest" | grep -oP '/tag/\K[^/]+')
            if [[ ! -n "$tag_version" ]]; then
    ...
    curl -4fLRo /usr/bin/x-ui-temp ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "x-ui/x-ui.db" "/etc/.gitignore"; then
                ...
                echo -e "${green}已将 x-ui.db 添加到 /etc/.gitignore 以供 etckeeper 使用${plain}"
            fi
        else
            echo "x-ui/x-ui.db" > "/etc/.gitignore"
            echo -e "${green}已创建 /etc/.gitignore 并添加 x-ui.db 以供 etckeeper 使用${plain}"
        fi
    fi
    
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.service.debian >/dev/null 2>&1
                ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.service.arch >/dev/null 2>&1
                ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service ${GH_PROXY}https://raw.githubusercontent.com/hxzlplp7/3x-ui/main/x-ui.service.rhel >/dev/null 2>&1
                ;;
            esac
