...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "${GH_PROXY}https://api.github.com/repos/hxzlplp7/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls "${GH_PROXY}https://api.github.com/repos/hxzlplp7/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
        ...
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz ${GH_PROXY}https://github.com/hxzlplp7/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="${GH_PROXY}https://github.com/hxzlplp7/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
